...
BQ27441::BQ27441() : _deviceAddress(BQ27441_I2C_ADDRESS), _sealFlag(false), _userConfigControl(false)
{
}
...
bool BQ27441::begin(void)
{
    ...
    // Create a file descriptor for the I2C bus
  	if ((file_i2c = open(DeviceName, O_RDWR)) < 0)
  	{
    ...
    if (ioctl(file_i2c, I2C_SLAVE, BQ27441_I2C_ADDRESS) < 0)
    {
...
// Reads and returns specified temperature measurement in Kelvin
...
bool BQ27441::i2cReadBytes(uint8_t subAddress, uint8_t * dest, uint8_t count)
{
  //int16_t timeout = BQ72441_I2C_TIMEOUT;

  write(file_i2c, &subAddress, 1);
  read(file_i2c, dest, count);

  return true;
}
  /*for (int i=0; i<count; i++)
  {
    read(file_i2c, &dest[i], 1);
  }*/
	/*int16_t timeout = BQ72441_I2C_TIMEOUT;
...
uint16_t BQ27441::i2cWriteBytes(uint8_t subAddress, uint8_t * src, uint8_t count)
{
  ...
  for (int i=0; i<count; i++)
   ...
   write(file_i2c, data, count + 1);
  /*
	write(file_i2c, &data, 3);
  write(file_i2c, &subAddress, 1);
  write(file_i2c, src, count);
...
BQ27441 lipo; // Use lipo.[] to interact with the library in an Arduino sketchm
